...
/* Define to 1 if you have the declaration of `__fpending', and to 0 if you
   don't. */
/* HobbyOS: fpending.h declares __fpending; the sysroot FILE
   layer implements it (unbuffered writes: always 0). */
...
/* HobbyOS: no <stdio_ext.h> in the sysroot. */
...
/* USE_UNLOCKED_IO: set to 0 in the HobbyOS overrides below */
...
/* ================= HobbyOS overrides =================
 *
 * Everything above is the configure output from the glibc host the
 * release was validated on; the block below retargets the interesting
 * feature switches at the HobbyOS libc.  Rules encoded here:
 *   - locale-wise HobbyOS IS a byte-mode C locale, so the wide/multibyte
 *     HAVE_* calls stay 1: our wchar.c/wctype.c implement the exact
 *     glibc C-locale semantics (bytes >= 0x80 are EILSEQ), which keeps
 *     sed's locale probes glibc-identical and lands it on its
 *     single-byte code paths.
 *   - anything the libc does not have (chown, popen yet, unlocked stdio,
 *     program_invocation_name) is turned off so gnulib picks the
 *     portable path.
 */

/* No chown(2) yet: sed's -i drops the best-effort ownership copy. */
#undef HAVE_FCHOWN

/* popen() lands with the shell-integration work: until then the
 * `e'/`s///e' commands report "not supported" (upstream's behavior on
 * hosts without popen). */
#undef HAVE_POPEN

/* Single-threaded stdio; the unlocked entry points do not exist.
 * unlocked-io.h decides per function via HAVE_DECL_*_UNLOCKED (it maps
 * the plain calls onto the *_unlocked ones when the host declares
 * them), so every probe must read "absent" to get the plain calls. */
...
/* No glibc-style program_invocation_name globals. */
...
/* Host-only probes that would pull glibc-only surface: treat as absent. */
#undef HAVE_FOLLOWER
...
#undef HAVE_EXECV
/* execv/execve exist as stubs in unistd.h; leave the gnulib probe off so
 * nothing builds on them until the kernel side lands. */
#undef HAVE_DUP3
#undef HAVE_PIPE2
#undef HAVE_FCHMODAT
...
#undef HAVE_VFORK_H
#undef HAVE_ALLOCA_H_VIA_STDLIB
#undef HAVE_MAP_ANONYMOUS
#undef HAVE_MAP_ANON

/* Keep the byte-exact GNU answers for the pieces that DO exist. */
#define HAVE_FOPEN_RT 1
#define ENABLE_NLS 0
#define HAVE_SETLOCALE 1
#define HAVE_LANGINFO_H 1
...
/* gnulib attribute macros.  On a full gnulib build these arrive from
 * gnulib's replacement headers (lib/stdio.h, lib/... .in.h shadows) that
 * sit earlier on the include path than the system headers.  HobbyOS uses
 * its own sysroot headers, so the macros are provided here.  Definitions
 * are copied verbatim from the release's gnulib sources. */
...
# if 4 < __GNUC__ + (3 <= __GNUC_MINOR__) || 3 < __clang_major__
/* _GL_ATTRIBUTE_ALLOC_SIZE: supplied by gnulib's xalloc.h. */
# else
#  define _GL_ATTRIBUTE_ALLOC_SIZE(args)
# endif
#endif
#ifndef _GL_ATTRIBUTE_CONST
# if 2 < __GNUC__ + (4 <= __GNUC_MINOR__)
#  define _GL_ATTRIBUTE_CONST __attribute__ ((__const__))
# else
...
# else
#  define _GL_ATTRIBUTE_SENTINEL
# endif
#endif
#ifndef _GL_ATTRIBUTE_FORMAT_PRINTF
# define _GL_ATTRIBUTE_FORMAT_PRINTF(formatstring_parameter, first_argument) \
   __attribute__ ((__format__ (__printf__, formatstring_parameter, first_argument)))
#endif
#ifndef _GL_UNUSED
# define _GL_UNUSED __attribute__ ((__unused__))
#endif
...
/* The real build passes -DO_BINARY=0 (non-Windows text=binary); mirror it
 * so binary-io.h's no-op __gl_setmode compiles. */
#define O_BINARY 0
#define O_TEXT 0

/* gnulib's strverscmp replacement: gnulib/strverscmp.c provides
 * __strverscmp, and gnulib's lib/string.in.h maps the public name onto
 * it.  We do not shadow <string.h>, so the mapping lives here. */
int strverscmp(const char *s1, const char *s2);

/* HobbyOS: no SELinux in the kernel. */
#undef HAVE_SELINUX_SELINUX_H
#undef HAVE_SELINUX_CONTEXT_H

/* HobbyOS: clang treats a function pointer carrying __attribute__((noreturn))
 * as a distinct type, rejecting obstack.c's plain-function initializer.
 * The annotation is only a compiler hint, so drop it for this port. */
#undef __attribute_noreturn__
#define __attribute_noreturn__ /* empty */

/* HobbyOS: the sysroot provides the GNU regex API and the (stub) SELinux
 * API directly, so gnulib's rpl_* indirections must not redirect calls
 * away from the plain names. */
#undef fgetfilecon
#undef getfilecon
#undef lgetfilecon
#undef re_comp
