...
/* HobbyOS port: faithful copy of the GNU textutils-2.1 tr.c with the
   ...
   static helper over the sysroot malloc (tr allocates only through
   xmalloc/xmemdup, so xrealloc/xstrdup are not needed), safe_read ->
   plain read() (the sysroot read() never returns EINTR), xstrtoul
   (lib/xstrtol.c) -> a local transcription of its unsigned-long path
   (tr passes NULL valid_suffixes, so no suffix multipliers can apply;
   base 8/10 repeat counts are all it is used for), stpcpy -> a local
   transcription of gnulib's lib/stpcpy.c (the sysroot has no stpcpy),
   close_stdout covered by exit()'s weak fflush(0), setlocale/
   bindtextdomain/textdomain dropped (single C locale), and the lone
   page-break form feed before usage() is dropped because tools/cstyle.py
   strips form feeds.  The GETOPT_HELP/VERSION option declarations are
   transcribed brace-less exactly as sys2.h writes them (the option array
   supplies the braces); cut_gnu.c wraps them in an extra pair, which
   silently zeroes the option's val and leaves --help/--version
   unreachable -- tr --version must identify textutils.  tr.c 2.1 has no
   wide-character code path at all, so there is no HAVE_MBRTOWC guard to
   write here: the single-byte C-locale path is the only path (the
   reference build in src/host/build_tu21_tr_ref.sh still sets
   HAVE_MBRTOWC/HAVE_WCHAR_H to 0 in its config.h).  The host build
   (Makefile tr_host) races this against the reference build via
   src/host/tr_parity.sh; byte-exact stdout is the bar.  */
...
/* Transcribed brace-less, exactly as textutils-2.1 sys2.h writes them:
   the option arrays supply the braces, as in {GETOPT_HELP_OPTION_DECL}.
   Wrapping these in another pair of braces turns the array element into a
   scalar initializer for the `name' field (val silently becomes 0) and
   --help/--version stop matching; keep them brace-less.  */
...
/* Allocating helpers in the spirit of the gnulib x* wrappers: the
   original code calls xmalloc/xmemdup and reaches xalloc_die on OOM.  */
...
/* stpcpy -- copy SRC to DEST, returning the address of the terminating
   NUL in DEST.  Transcribed from gnulib's lib/stpcpy.c; the sysroot has
   no stpcpy, and tr uses it only in make_printable_str.  */
...
/* The ctype predicate set is transcribed from the textutils-2.1
   src/sys2.h macros (STDC_HEADERS there asserted the ctype functions are
   safe for every character value, so IN_CTYPE_DOMAIN folds to 1).  */
...
static ssize_t safe_read (int desc, void *ptr, size_t len) {
  return read (desc, ptr, len);
}

/* xstrtoul -- transcribed from textutils-2.1 lib/xstrtol.c, whose
   lib/xstrtoul.c instantiation just #defines __strtol to strtoul and
   includes it.  tr always passes valid_suffixes == NULL, which the
   original treats as `allow any suffix' and returns LONGINT_OK without
   scaling, so only that path is carried over.  */
enum strtol_error {
  LONGINT_OK, LONGINT_INVALID, LONGINT_INVALID_SUFFIX_CHAR, LONGINT_OVERFLOW
};

static enum strtol_error xstrtoul (const char *s, char **ptr, int strtol_base, unsigned long int *val, const char *valid_suffixes) {
  char *t_ptr;
  char **p;
  ...
  /* An unsigned conversion may not have a leading `-'.  */
  q = s;
  ...
  /* tr never takes this path; suffix scaling is not carried over.  */
  if (**p != '\0')
...
static struct option const long_options[] = { {"complement", no_argument, NULL, 'c'}, {"delete", no_argument, NULL, 'd'}, {"squeeze-repeats", no_argument, NULL, 's'}, {"truncate-set1", no_argument, NULL, 't'}, {GETOPT_HELP_OPTION_DECL}, {GETOPT_VERSION_OPTION_DECL}, {NULL, 0, NULL, 0}
};
...
static int is_equiv_class_member (unsigned int equiv_class, unsigned int c) {
...
static int is_char_class_member (enum Char_class char_class, unsigned int c) {
  int result;
...
static int unquote (const unsigned char *s, struct E_string *es) {
  size_t i, j;
  size_t len;
...
static enum Char_class look_up_char_class (const unsigned char *class_str, size_t len) {
  unsigned int i;
...
static void append_normal_char (struct Spec_list *list, unsigned int c) {
...
static int append_range (struct Spec_list *list, unsigned int first, unsigned int last) {
  struct List_element *new;
...
static int append_char_class (struct Spec_list *list,
...
static void append_repeated_char (struct Spec_list *list, unsigned int the_char,
                      size_t repeat_count) {
  struct List_element *new;
...
static int append_equiv_class (struct Spec_list *list,
...
static int find_closing_delim (const struct E_string *es, size_t start_idx,
...
static int star_digits_closebracket (const struct E_string *es, size_t idx) {
  size_t i;
...
static int build_spec_list (const struct E_string *es, struct Spec_list *result) {
...
static void get_s2_spec_stats (struct Spec_list *s2, size_t len_s1) {
  get_spec_stats (s2);
...
static int parse_str (const unsigned char *s, struct Spec_list *spec_list) {
  struct E_string es;
  int fail;
...
static int homogeneous_spec_list (struct Spec_list *s) {
  int b, c;
...
static void squeeze_filter (unsigned char *buf, size_t size, Filter reader) {
...
static size_t read_and_delete (unsigned char *buf, size_t size, Filter not_used) {
  size_t n_saved;
...
static size_t read_and_xlate (unsigned char *buf, size_t size, Filter not_used) {
...
static void set_initialize (struct Spec_list *s, int complement_this_set, SET_TYPE *in_set) {
  int c;
  size_t i;
...
int main (int argc, char **argv) {
  ...
  /* HobbyOS port: setlocale/bindtextdomain/textdomain are dropped (the
     sysroot has a single C locale), and so is atexit (close_stdout) --
     exit() flushes stdout through its weak fflush(0).  */
