...
#endif
#define XARGMATCH(a, b, s, m) (argmatch_panic ((a), (b)))
...
#define case_GETOPT_VERSION_CHAR(Program_name, Authors) \
  case GETOPT_VERSION_CHAR: \
    ...
    exit (0); break
#define SETVBUF(s, b, m, n) setvbuf ((s), (b), (m), (n))
#define LONGEST_HUMAN_READABLE 30
#define MIN(a, b) ((a) < (b) ? (a) : (b))
#define MAX(a, b) ((a) > (b) ? (a) : (b))

/* HobbyOS port helper: parse an unsigned decimal with an optional single
   b/k/m suffix (like gnulib's xstrtoumax(...,"bkm")).  0 ok, -1 too big,
   -2 invalid suffix/characters.  */
static int parse_uintmax_units (const char *s, const char *allowed_suffixes, uintmax_t *out) {
  uintmax_t n, mult = 1;
  char *end;
  errno = 0;
  n = (uintmax_t) strtoull (s, &end, 10);
  if (errno == ERANGE && n == UINTMAX_MAX)
    return -1;
  if (*end != '\0' && end[1] == '\0'
      && strchr (allowed_suffixes, *end) != 0) {
    switch (*end) {
    case 'b': mult = 512; break;
    case 'k': mult = 1024; break;
    case 'm': mult = 1024 * 1024; break;
    default: mult = 1;      /* e.g. 'c': accepted, no multiplier */
    }
  } else if (*end != '\0')
    return -2;
  if (mult == 0 || (mult != 1 && n > UINTMAX_MAX / mult))
    return -1;
  *out = n * mult;
  return 0;
}

static int argmatch_panic (const char *opt, const char *arg) {
  error (EXIT_FAILURE, 0, _("invalid argument `%s' for `%s'"), arg, opt);
  return 0;
}
...
static void xwrite (int fd, char *const buffer, size_t n_bytes) {
...
static long dump_remainder (const char *pretty_filename, int fd, off_t n_bytes) {
  char buffer[BUFSIZ];
  int bytes_read;
  long n_written;
...
static void xlseek (int fd, off_t offset, int whence, char const *filename) {
  ...
  if (offset < 0)
    offset = -offset;  {
    unsigned long long abso = (unsigned long long)
    (offset < 0 ? -(offset) : offset);
    char d[24];
    int di = 23;
    d[di--] = '\0';
    if (abso == 0) d[di--] = '0';
    while (abso) { d[di--] = (char)('0' + abso % 10); abso /= 10; }
    strcpy (buf, &d[di + 1]);
  }
  s = buf;
  switch (whence) {
...
static int file_lines (const char *pretty_filename, int fd, long int n_lines,
...
static int pipe_lines (const char *pretty_filename, int fd, long int n_lines) {
  struct linebuffer {
    int nbytes, nlines;
    char buffer[BUFSIZ];
  ...
  while ((nbytes = tmp->nbytes = read (fd, tmp->buffer, BUFSIZ)) > 0) {
    tmp->nlines = 0;
    tmp->next = NULL;
...
static int pipe_bytes (const char *pretty_filename, int fd, off_t n_bytes) {
  struct charbuffer {
    int nbytes;
    char buffer[BUFSIZ];
  ...
  /* Input is always read into a fresh buffer.  */
  while ((tmp->nbytes = read (fd, tmp->buffer, BUFSIZ)) > 0) {
    tmp->next = NULL;
...
static int start_bytes (const char *pretty_filename, int fd, off_t n_bytes) {
  char buffer[BUFSIZ];
  int bytes_read = 0;

  while (n_bytes > 0 && (bytes_read = read (fd, buffer, BUFSIZ)) > 0)
    n_bytes -= bytes_read;
  if (bytes_read == -1) {
    error (0, errno, "%s", pretty_filename);
    return 1;
...
static int start_lines (const char *pretty_filename, int fd, long int n_lines) {
  char buffer[BUFSIZ];
  int bytes_read = 0;
  int bytes_to_skip = 0;
  ...
  while (n_lines && (bytes_read = read (fd, buffer, BUFSIZ)) > 0) {
    bytes_to_skip = 0;
...
static void recheck (struct File_spec *f) {
...
static unsigned int n_live_files (const struct File_spec *f, int n_files) {
  int i;
...
static int tail_bytes (const char *pretty_filename, int fd, off_t n_bytes) {
  struct stat stats;
...
static int tail_lines (const char *pretty_filename, int fd, long int n_lines) {
  struct stat stats;
...
static int tail (const char *pretty_filename, int fd, off_t n_units) {
  if (count_lines)
...
static int tail_file (struct File_spec *f, off_t n_units) {
  int fd, errors;
...
static int parse_obsolescent_option (int argc, const char *const *argv,
  ...
  /* HobbyOS has no POSIX-200112 mandate; accept the old syntax. */
  obsolete_usage = 1;
  ...
  *fail = 0;
  if (n_string == NULL)
    *n_units = DEFAULT_N_LINES;
    ...
    /* The old syntax's trailing letter (c/b/l) is itself the format
       selector, but xstrtoumax still had to accept it: pass it as the
       allowed single-char suffix so "3c" parses as 3 bytes and "3b"
       as 3*512, matching the original.  */
    {
      char suffix_buf[2] = { (char) *n_string_end, '\0' };
      int p_err = parse_uintmax_units (n_string, suffix_buf, &tmp);
      if (p_err == 0 && tmp <= OFF_T_MAX)
        *n_units = (off_t) tmp;
      else {
        /* Extract a NUL-terminated string for the error message.  */
        size_t len = n_string_end - n_string;
        char *n_string_tmp = malloc (len + 1);
...
static void parse_options (int argc, char **argv,
  ...
  while ((c = getopt_long (argc, argv, "c:n:fFqs:v", long_options, NULL))
    ...
    case 'n':
      ...
      {    uintmax_t n;
        int p_err = parse_uintmax_units (optarg, "bkm", &n);
        if (p_err == -2) {
    ...
    case MAX_UNCHANGED_STATS_OPTION:
      /* --max-unchanged-stats=N */    if (parse_uintmax_units (optarg, "",
                         (uintmax_t *) &max_n_unchanged_stats_between_opens) != 0) {
        error (EXIT_FAILURE, 0,
    ...
    case MAX_CONSECUTIVE_SIZE_CHANGES_OPTION:
      /* --max-consecutive-size-changes=N */    if (parse_uintmax_units (optarg, "",
                         (uintmax_t *) &max_n_consecutive_size_changes_between_opens)
    != 0) {
        error (EXIT_FAILURE, 0,
    ...
    case PID_OPTION: {    uintmax_t tmp_umax;
        if (parse_uintmax_units (optarg, "", &tmp_umax) != 0 || tmp_umax > PID_T_MAX) {
    ...
    case 's': {    uintmax_t tmp_umax;
        if (parse_uintmax_units (optarg, "", &tmp_umax) != 0 || tmp_umax > UINT_MAX) {
          error (EXIT_FAILURE, 0,
                 _("%s: invalid number of seconds"), optarg);
        }
        sleep_interval = (unsigned int) tmp_umax;
      }
      break;
...
int main (int argc, char **argv) {
  ...
  F = (struct File_spec *) malloc (n_files * sizeof (F[0]));
  for (i = 0; i < n_files; i++)
    F[i].name = file[i];
