...
/* HobbyOS port: faithful copy of the GNU textutils-2.1 expand.c with the
   gnulib scaffolding replaced by the Phase-2/3 sysroot: error() from
   error.h/error.c, getopt_long from our getopt.h, xrealloc/xalloc_die ->
   local helpers over the sysroot malloc, system.h's ISBLANK/ISDIGIT/STREQ
   inlined below, posix2_version() transcribed from lib/posixver.c at the
   ...
   SET_BINARY2 is a no-op (no text/binary stream distinction).  The host
   build (Makefile expand_host) races this against the reference build in
   src/host/expand_parity.sh; byte-exact output is the bar.  */
...
/* The shared --help/--version option scaffolding, transcribed from
   src/sys2.h (the macro definitions the original build gets from there).
...
/* Text/binary stream distinction does not exist here.  */
...
static struct option const longopts[] = { {"tabs", required_argument, NULL, 't'}, {"initial", no_argument, NULL, 'i'}, {GETOPT_HELP_OPTION_DECL}, {GETOPT_VERSION_OPTION_DECL}, {NULL, 0, NULL, 0}
};
...
static void parse_tabstops (char *stops) {
  int tabval = -1;
...
static void validate_tabstops (int *tabs, int entries) {
  int prev_tab = 0;
  int i;
...
/* posix2_version -- which POSIX version to conform to, for utilities.
   ...
   as expand's gate on the obsolete `-LIST' tab syntax.  The compile-time
