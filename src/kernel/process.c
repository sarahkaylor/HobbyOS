...
void schedule(struct trap_frame *tf) {
    ...
    while (1) {
        ...
        int any_alive = 0;
        for (int i = 0; i < MAX_PROCESSES; i++) {
            if (proc_table[i].state != PROC_STATE_FREE && proc_table[i].state != PROC_STATE_EXITED) {
                any_alive = 1;
                break;
            }
        }
...
void process_exit(struct trap_frame *tf) {
    ...
    uart_puts("[KERNEL] Process "); print_int(cur->pid); uart_puts(" exited unexpectedly or gracefully.\n");
...
void process_sleep(void) {
    ...
    // Actually, I'll just change the state and the next schedule() call (from timer or exit) 
    // will skip this process. But I want to block IMMEDIATELY.
