#!/bin/bash
# build_tu21_paste_ref.sh — build GNU textutils-2.1's ORIGINAL paste as a strict
# byte-exact reference for the HobbyOS paste port (src/host/paste_parity.sh).
#
...
# the small set of .c files paste needs. paste.c calls only xmalloc/
# xrealloc (no getstr/safe-read/full-write), so the file list is cut's
# minus lib/getstr.c. Output: $OUT (default /tmp/tu21_paste,
# formally /tmp/gnupaste-ref/tu21_paste).
#
# Usage: bash src/host/build_tu21_paste_ref.sh [output-path]
set -e
BASE="$(cd "$(dirname "$0")/../../third_party/textutils-2.1" && pwd)"
OUT="${1:-/tmp/gnupaste-ref/tu21_paste}"
WD="$(mktemp -d /tmp/tu21ref.XXXXXX)"
trap 'rm -rf "$WD"' EXIT

cat > "$WD/config.h" <<'EOF'
/* minimal config for host-compiling textutils-2.1 src/paste.c against
...
# sys_errlist/sys_nerr + other vanished glibc bits (see build_tu21_wc_ref.sh)
cat > "$WD/stubs.c" <<'EOF'
#include <stdio.h>
#include <stdarg.h>
#include <stddef.h>
...
/* lib/xmalloc.c needs 1999-era autoconf malloc probes; provide the four
   entry points paste.c actually calls, with 2.1-faithful OOM behavior. */
#include <stdlib.h>
