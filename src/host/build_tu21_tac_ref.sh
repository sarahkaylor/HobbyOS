#!/bin/bash
# build_tu21_tac_ref.sh — build GNU textutils-2.1's ORIGINAL tac as a
# strict byte-exact reference for the HobbyOS tac port
# (src/host/tac_parity.sh).
#
...
# against a minimal hand-rolled config.h.  tac's gnulib surface:
# safe-read.c (safe_read: read-with-EINTR-retry), getopt+getopt1 (getopt_long
# for parse_long_options and the option loop), error.c, closeout.c
# (atexit(close_stdout)) and long-options.c (--help/--version).  Output:
# $OUT (default /tmp/gnutac-ref/tu21_tac).
#
# Usage: bash src/host/build_tu21_tac_ref.sh [output-path]
set -e
BASE="$(cd "$(dirname "$0")/../../third_party/textutils-2.1" && pwd)"
OUT="${1:-/tmp/gnutac-ref/tu21_tac}"
WD="$(mktemp -d /tmp/tu21tacref.XXXXXX)"
trap 'rm -rf "$WD"' EXIT

cat > "$WD/config.h" <<'EOF'
/* minimal config for host-compiling textutils-2.1 src/tac.c against
...
/* version_etc lives in lib/version-etc.c, which drags in the full
   copyright/authors banner; tac's --version only needs this one line,
   printed exactly as the ported tac prints it. */
...
/* closeout.c quotes the output name on a write error; tac's error paths
...
/* lib/xmalloc.c's autoconf probes are not worth running; provide the
   entry points tac.c/safe-read.c call, with 2.1-faithful OOM behavior. */
