#!/bin/bash
# build_tu21_expand_ref.sh — build GNU textutils-2.1's ORIGINAL expand as a
# strict byte-exact reference for the HobbyOS expand port
# (src/host/expand_parity.sh).
#
...
# the small set of .c files expand needs.  expand.c differs from cut.c in
# exactly one gnulib dependency: it gates the obsolete `-LIST' tab syntax
# on posix2_version() ("`-LIST' option is obsolete; use `-t LIST'"), so
# lib/posixver.c joins the file list (getstr.c is cut-only and is not
# needed).  NOTE: posixver.c takes _POSIX2_VERSION from the system's
# <unistd.h>, and modern glibc hard-defines it to 200809L (a config.h
# define is overridden), so this reference REJECTS `-8'/`-4,8' by default
# while the port accepts them (HobbyOS's sysroot publishes no
# _POSIX2_VERSION, i.e. 0 -- the same convention head/tail/fold use).  The
# harness therefore runs the obsolete-syntax cases with _POSIX2_VERSION=0
# in the environment, which posixver.c's getenv() path honors at runtime
# for both implementations.  Output: $OUT (default /tmp/tu21_expand,
# formally /tmp/gnuexpand-ref/tu21_expand).
#
# Usage: bash src/host/build_tu21_expand_ref.sh [output-path]
set -e
BASE="$(cd "$(dirname "$0")/../../third_party/textutils-2.1" && pwd)"
OUT="${1:-/tmp/gnuexpand-ref/tu21_expand}"
WD="$(mktemp -d /tmp/tu21ref.XXXXXX)"
trap 'rm -rf "$WD"' EXIT

cat > "$WD/config.h" <<'EOF'
/* minimal config for host-compiling textutils-2.1 src/expand.c against
...
/* lib/xmalloc.c needs 1999-era autoconf malloc probes; provide the four
   entry points expand.c actually calls (xrealloc), with 2.1-faithful OOM
   behavior. */
#include <stdlib.h>
