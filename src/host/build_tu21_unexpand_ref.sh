#!/bin/bash
# build_tu21_unexpand_ref.sh — build GNU textutils-2.1's ORIGINAL unexpand as
# a strict byte-exact reference for the HobbyOS unexpand port
# (src/host/unexpand_parity.sh).
#
...
# the small set of .c files unexpand needs. Output: $OUT (default
# /tmp/gnuunexpand-ref/tu21_unexpand).
#
# unexpand's extra gnulib surface vs cut: lib/posixver.c (the
# posix2_version() gate behind the obsolete `-LIST' diagnostic) — glibc's
# <unistd.h> defines _POSIX2_VERSION there, so this reference reports
# 200809 and rejects `-4'-style lists, exactly like the port does.
#
# Usage: bash src/host/build_tu21_unexpand_ref.sh [output-path]
set -e
BASE="$(cd "$(dirname "$0")/../../third_party/textutils-2.1" && pwd)"
OUT="${1:-/tmp/gnuunexpand-ref/tu21_unexpand}"
WD="$(mktemp -d /tmp/tu21ref.XXXXXX)"
trap 'rm -rf "$WD"' EXIT

cat > "$WD/config.h" <<'EOF'
/* minimal config for host-compiling textutils-2.1 src/unexpand.c against
...
/* lib/xmalloc.c needs 1999-era autoconf malloc probes; provide the four
   entry points unexpand.c actually calls, with 2.1-faithful OOM behavior. */
#include <stdlib.h>
