#!/bin/bash
# build_tu21_comm_ref.sh — build GNU textutils-2.1's ORIGINAL comm as a strict
# byte-exact reference for the HobbyOS comm port (src/host/comm_parity.sh).
#
...
# the small set of .c files comm needs: comm.c itself plus the lib
# helpers it calls -- linebuffer.c (initbuffer/readline), getopt.c /
# getopt1.c (getopt_long), error.c (error), closeout.c (atexit'd
# close_stdout) and hard-locale.c (hard_locale).  Calling xmemcoll is
# dead with HAVE_SETLOCALE 0, so memcoll.c/quotearg.c are not needed.
# Output: $OUT (default /tmp/tu21_comm).
#
# Usage: bash src/host/build_tu21_comm_ref.sh [output-path]
set -e
BASE="$(cd "$(dirname "$0")/../../third_party/textutils-2.1" && pwd)"
OUT="${1:-/tmp/tu21_comm}"
WD="$(mktemp -d /tmp/tu21commref.XXXXXX)"
trap 'rm -rf "$WD"' EXIT

cat > "$WD/config.h" <<'EOF'
/* minimal config for host-compiling textutils-2.1 src/comm.c against
...
/* HobbyOS comm-parity choice: 0, not 1.  comm.c only reaches xmemcoll
   under (HAVE_SETLOCALE && hard_LC_COLLATE), and src/host/comm_parity.sh
   pins LC_ALL=C for both sides, where hard_locale(LC_COLLATE) is 0 for
   the real 2.1 binary too.  With 0 the reference takes the same memcmp
   (C-collation) path the port does, the branch is compile-time dead and
   the gnulib memcoll/quotearg graph stays out of this build.  setlocale
   in comm.c's main is still compiled and called (<locale.h> is real on
   the host), so the LC_COLLATE behavior under other locales is simply
   not exercised -- the harness never runs it there. */
...
/* lib/xmalloc.c needs 1999-era autoconf malloc probes; provide the entry
   points lib/linebuffer.c actually calls, with 2.1-faithful OOM
   behavior.  (closeout.c's write-error path also reaches quotearg_colon
   and error(), both covered above/below.)  */
#include <stdlib.h>
