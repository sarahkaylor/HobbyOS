#!/bin/bash
# build_tu21_cut_ref.sh — build GNU textutils-2.1's ORIGINAL cut as a strict
# byte-exact reference for the HobbyOS cut port (src/host/cut_parity.sh).
#
...
# the small set of .c files cut needs. Output: $OUT (default
# /tmp/tu21_cut, formally /tmp/gnucut-ref/tu21_cut).
#
# Usage: bash src/host/build_tu21_cut_ref.sh [output-path]
set -e
BASE="$(cd "$(dirname "$0")/../../third_party/textutils-2.1" && pwd)"
OUT="${1:-/tmp/gnucut-ref/tu21_cut}"
WD="$(mktemp -d /tmp/tu21ref.XXXXXX)"
trap 'rm -rf "$WD"' EXIT

cat > "$WD/config.h" <<'EOF'
/* minimal config for host-compiling textutils-2.1 src/cut.c against
...
/* lib/xmalloc.c needs 1999-era autoconf malloc probes; provide the four
   entry points cut.c actually calls, with 2.1-faithful OOM behavior. */
#include <stdlib.h>
