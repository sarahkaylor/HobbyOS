#!/bin/bash
# build_tu21_tr_ref.sh — build GNU textutils-2.1's ORIGINAL tr as a strict
# byte-exact reference for the HobbyOS tr port (src/host/tr_parity.sh).
#
...
# the small set of .c files tr needs. Output: $OUT (default
# /tmp/gnutr-ref/tu21_tr).
#
# Usage: bash src/host/build_tu21_tr_ref.sh [output-path]
set -e
BASE="$(cd "$(dirname "$0")/../../third_party/textutils-2.1" && pwd)"
OUT="${1:-/tmp/gnutr-ref/tu21_tr}"
WD="$(mktemp -d /tmp/tu21trref.XXXXXX)"
trap 'rm -rf "$WD"' EXIT

cat > "$WD/config.h" <<'EOF'
/* minimal config for host-compiling textutils-2.1 src/tr.c against
...
# NOTE: unlike cut, tr genuinely links lib/xstrtoul.c for its [c*n] repeat
# counts, so there is deliberately no xstrtoul stub here (that would be a
# duplicate definition at link time) and stpcpy comes from libc via the
# HAVE_DECL_STPCPY/HAVE_STPCPY=0 settings above.
cat > "$WD/stubs.c" <<'EOF'
#include <stdio.h>
#include <stdarg.h>
#include <stddef.h>
...
/* lib/xmalloc.c needs 1999-era autoconf malloc probes; provide the four
   entry points tr.c actually calls, with 2.1-faithful OOM behavior. */
#include <stdlib.h>
