...
- **Next blocker (task: GSP-RM init / Xid 79):** GSP-RM bring-up doesn't complete. Likely (a) some BAR reads return `0xFFFFFFFF` (proxy returns that for >16 MB BAR0 or on read timeout) → driver declares GPU off-bus; (b) GSP RPC message-queue coherence too slow — the `/proc/PID/mem` bidi-diff syncs at a 1 s cadence but RPC round-trips need ms; (c) interrupt delivery to RPC completion. This is the deep GPU-bring-up correctness layer, distinct from firmware load and read latency.
