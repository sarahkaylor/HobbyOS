...
### E. GSP Firmware Footprint & Full-Mirror Requirement (RTX 4090 / AD102)
- **Deployment topology (measured):** VM 225 ("testvm", Linux) is the consumer that runs `nvidia-smi`; its GPU is a `net_pci_client` vfio-user device on the Proxmox host that forwards over UDP/RDMA to VM 205 ("HobbyOSHostGPU"), which owns the physical card (`0000:01:00`, `0x10de:0x2684`). `net_pci_client` also writes VM 205's RAM directly via `/proc/<205-pid>/mem`.
- **Firmware is NOT one contiguous blob.** On disk `gsp_ga10x.bin` (shared by Ampere/Ada) is ≈30–50 MB depending on driver version. At runtime the driver decompresses it and scatters a radix3 page table + WPR2 + libos heap + RPC queues across guest RAM, so the GPU's DMA working set spans the **entire populated guest RAM**, not a fixed region.
- **Guest RAM is registered as MULTIPLE regions (measured from the vIOMMU DMA registrations).** For VM 225 (3 GB): a low region `iova=0xc0000` len≈2047 MB **and** a high region `iova=0x100000000` len=1024 MB (the PCI hole splits RAM at 2 GB). Therefore the mirror MUST cover every region — the prior single-`g_main_ram` mirror captured only the first/largest one, so any firmware page in the high 1 GB was never mirrored and the GPU DMA-read zeros.
- **Implementation:** `net_pci_client.c` tracks every >1 MB region in `g_ram_regions[]` (`ram_region_register`); the direct `/proc/PID/mem` blast writes all of them (measured: 8 regions incl. overlaps, ~9.2 GB written in ~2.9 s, 0 errors). `blast_verify_regions()` does a **sampled, LOG-ONLY** CRC32 check (3 non-zero 2 MB spans per region) via `RDMA_OP_DMA_SYNC_RELIABLE` before falcon start. It deliberately does NOT repair: by then the GPU/driver are live, so a per-page guest→host overwrite would clobber legitimate GPU DMA writes (the bidi-diff loop owns host↔guest reconciliation). A full per-2 MB verify (≈700 spans) previously starved the driver's BAR polls and tripped the guest soft-lockup watchdog — keep it sampled.
- **MEASURED firmware footprint (point #7):** `FIRMWARE FOOTPRINT (non-zero) = 1409 MB` on a 3 GB guest — i.e. the GPU's DMA working set spans well over a gigabyte of scattered guest RAM (vastly larger than the ~30–50 MB on-disk `gsp` file). A fixed-size/single-region assumption truncates it; the full multi-region mirror is mandatory. The host confirms it with dozens of `[RDMA] DMA VERIFY OK` lines.
- **How far it gets now:** firmware loads → GSP falcon starts (`deferred BAR write offset=0x840100 val=0x2`) → GSP runs and DMA-writes ~77 MB of output back (`BIDI SYNC pass 0: rev=39(76912KB)`), then steady 2–4 MB/pass. This is the furthest the system has reached.
- **Read latency — ROOT-CAUSED AND FIXED.** The per-BAR-read slowness (which soft-locked the guest, `CPU#N stuck`) was caused by a synchronous `printf` of an `[RDMA_TX]` trace line on EVERY read attempt in `do_rdma_transaction` — writing to a logfile that grew to 200 MB+ blocked for hundreds of ms and *caused* the 500 ms timeouts/retries. RTT instrumentation proved it: ungated = ~785 µs... actually ~785 **ms** avg/read with 59% retried; gated = **~0.2 ms avg, 100% first-try, 0 retries**. The trace is now sampled (first 30 + every 2000th). NOTE: do NOT shorten the 500 ms timeout or add retries — an attempt to do so REGRESSED to universal timeouts because re-sends create duplicate host responses that desync the req/resp stream. The 500 ms timeout is fine now that reads complete in 0.2 ms.
- **Current state (2026-06-09):** reads fast, guest boots fully, `nvidia-smi` RUNS (no hang) but prints **"No devices were found"**. Guest dmesg (driver 595.71.05): GSP firmware boots, then **`NVRM: Xid 79, GPU has fallen off the bus`** at ~14 s → `RmInitAdapter failed! (0x62:0x40:2830)` → `nvidia-drm: Failed to allocate NvKmsKapiDevice`. (Guest login is on VGA tty1, not serial — `qm terminal`/`test_nvidia.py` can't reach it; drive it with `qm sendkey` + `qm monitor screendump`.)
- **Next blocker (task: GSP-RM init / Xid 79) — DIAGNOSED to two architectural gaps:**
  1. **No real interrupts.** The host forwards interrupts by polling `PMC_INTR` (BAR0+0x100), but on this Ada GSP GPU that register returns NVIDIA's PRI **poison `0xbadf5040`** — it's GSP-locked, and real GSP/RM interrupts are **MSI-X**, which HobbyOS doesn't capture. Forwarding was made level-triggered (mechanically works: `irq_fwd` 1→439) but only delivered poison, so a `0xbadfXXXX` poison filter was added to stop spamming fake IRQs. Net: the guest gets no real interrupts → GSP RPC completion is never signaled → `RmInitAdapter` times out. Real fix needs **host-side MSI-X capture + forwarding** for the physical GPU.
  2. **RPC coherence + write clobbering.** The `/proc/PID/mem` bidi-diff (host↔guest RAM mirror) uses "host wins" on 4 KB conflicts, which **overwrites guest RPC command writes** (confirmed: `clobber=17` pages at `0x21203000` while the guest wrote RPC at `0x20ec0000`). Coherence floor is ~370–390 ms (full 2 GB scan per pass; the inter-pass sleep was cut 1 s→20 ms). Real fix needs a **fast, non-clobbering dedicated sync channel** for the small GSP RPC-queue regions (sub-ms, never overwrite guest-authored command pages).
  - Both are substantial new infrastructure, distinct from firmware load and read latency. Instrumentation is in place (`BIDI SYNC ... clobber=/fwd_off=`, `[RTT STATS]`) to guide the work.

- **UPDATE — broke past the Xid 79 wall (sustained GSP RPC).** Four fixes got the guest driver from "GPU off bus at 14s, `fwd=0`" to **sustained bidirectional GSP RPC** (1087 fwd>0 passes, ran to bidi pass 9809, RPC at guest off `0x28cc0000` ongoing to end; login prompt now clean):
  1. **Read-latency variance ROOT CAUSE (supersedes the earlier printf theory):** `provider_loop` read poison-locked `PMC_INTR` (BAR0+0x100) **every iteration**; once GSP locks that reg the MMIO read incurs a slow PRI timeout, throttling the single-threaded host loop → ~785 ms/read + 59% retries (variable run-to-run). **Fix: rate-limit the 0x100 poll to every 50 ms** → reads stably ~0.2 ms, 0 retries.
  2. **Dedup overlapping RAM regions** (guest maps the same memfd at iova 0/0xf0000/0x100000/0xc0000) → blast 9.2 GB/2.9 s → 3 GB/1.6 s. Confirmed the **high region holds 749 MB of firmware** (multi-region mirror is essential).
  3. **Adaptive hot-set bidi sync** (sync only recently-changed 2 MB chunks on fast passes; full scan every 16) → RPC-region coherence ~390 ms → **~3 ms**.
  4. **Synthesized guest interrupts from GPU DMA (`rev`) activity** (set `g_pending_irq_mask` when the GPU writes host RAM).
  - **Still NOT confirmed: `nvidia-smi`.** The guest VGA console (tty1) **wedges** (a vCPU stalls mid-printk during sustained RPC — possible residual soft-lockup), and the login getty is VGA-only (serial unused), so the shell can't be driven to run `nvidia-smi`. Next: give VM 225 a serial console (`console=ttyS0`) or otherwise get a reliable shell; investigate the vCPU stall (tune synth-IRQ rate / check IRQ storm); confirm `RmInitAdapter` completes.
